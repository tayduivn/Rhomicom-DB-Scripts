/* Formatted on 10/6/2014 3:13:56 PM (QP5 v5.126.903.23003) */
-- FUNCTION: APLAPPS.GET_ITEM_UNIT_PRICE_LS_TX(NUMBER, NUMBER)

-- DROP FUNCTION APLAPPS.GET_ITEM_UNIT_PRICE_LS_TX(NUMBER, NUMBER);

CREATE OR REPLACE FUNCTION APLAPPS.GET_ITEM_UNIT_PRICE_LS_TX (
   ITMID    NUMBER,
   QTY      NUMBER
)
   RETURN NUMBER
...
BEGIN
   ...
   SELECT   COUNT (A.UOM_ID)
    ...
    WHERE   (    A.ITEM_ID = ITMID
             ...
             AND A.PRICE_LESS_TAX != 0);

   IF QTY <= 1 OR UOMCNT <= 0
   THEN
      SELECT   NVL (A.ORGNL_SELLING_PRICE, 0), BASE_UOM_ID
